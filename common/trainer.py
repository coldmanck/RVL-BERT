...
def train(net,
    ...
    for epoch in range(begin_epoch, end_epoch):
        ...
        for nbatch, batch in enumerate(train_loader):
            ...
            # ONLY FOR DEBUG PURPOSE. COMMENT BEFORE TRAINING
            if debug:
                print('DEBUG mode is on! Break after only 1 iteration')
                break

        # excute epoch_end_callbacks
        if validation_monitor is not None and epoch % eval_epoch == 0:
            print('Validating...')
            if clear_cache is None:
                validation_monitor(epoch, net, optimizer, writer)
            else:
                validation_monitor(epoch, net, optimizer, writer, clear_cache)
        if testing_monitor is not None:
            print('Testing...')
            if clear_cache is None:
                testing_monitor(epoch, net, optimizer, writer)
            else:
                testing_monitor(epoch, net, optimizer, writer, clear_cache)

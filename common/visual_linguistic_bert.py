...
class VisualLinguisticBert(BaseModel):
    ...
    def forward(self,
        ...
        embedding_output, attention_mask, text_mask_new, object_mask_new, spo_fused_masks = self.embedding(text_input_ids,
                                                                                          text_token_type_ids,
                                                                                          text_visual_embeddings,
                                                                                          text_mask,
                                                                                          object_vl_embeddings,
    ...
    def embedding(self,
        ...
        if self.enhanced_img_feature and object_visual_feat is not None and spo_len is not None:
            ...
            object_visual_feat = object_visual_feat.view(-1, object_visual_feat.shape[2], object_visual_feat.shape[3], object_visual_feat.shape[4])
            ...
            object_visual_feat = object_visual_feat.view((object_visual_feat_shape[0], object_visual_feat_shape[1], 768, object_visual_feat_shape[3], object_visual_feat_shape[4]))
            ...
            if self.no_predicate: # VRD
                for i in range(text_input_ids.shape[0]): # for each img sample
                    ...
                    sub_text_emb = text_linguistic_embedding[i, 1:subj_end].view(-1, 1, 1) if spo_len[i, 0] == 1 else text_linguistic_embedding[i, 1:subj_end].sum(dim=0).view(-1, 1, 1)
                    ...
                    obj_text_emb = text_linguistic_embedding[i, subj_end:obj_end].view(-1, 1, 1) if spo_len[i, 1] == 1 else text_linguistic_embedding[i, subj_end:obj_end].sum(dim=0).view(-1, 1, 1)
                    ...
                    text_visual_embeddings[i, 1:subj_end] = self.lan_img_avgpool(object_visual_feat[i,0] * spo_fused_norm[0]).squeeze() # spo_fused[0]
                    text_visual_embeddings[i, subj_end:obj_end] = self.lan_img_avgpool(object_visual_feat[i,0] * spo_fused_norm[1]).squeeze() # spo_fused[1]
            else: # SpatialSense
                for i in range(text_input_ids.shape[0]): # for each img sample
                    subj_end = 1 + spo_len[i, 0]
                    ...
                    pred_text_emb = text_linguistic_embedding[i, subj_end:pred_end].view(-1, 1, 1) if spo_len[i, 1] == 1 else text_linguistic_embedding[i, subj_end:pred_end].sum(dim=0).view(-1, 1, 1)
                    ...
                    obj_text_emb = text_linguistic_embedding[i, pred_end:obj_end].view(-1, 1, 1) if spo_len[i, 2] == 1 else text_linguistic_embedding[i, pred_end:obj_end].sum(dim=0).view(-1, 1, 1)
                    obj_text_emb = torch.cat(([obj_text_emb] * 14), dim=1)
                    obj_text_emb = torch.cat(([obj_text_emb] * 14), dim=2)
                    fused_obj = object_visual_feat[i,0] + obj_text_emb
                    ...
                    text_visual_embeddings[i, 1:subj_end] = self.lan_img_avgpool(object_visual_feat[i,0] * spo_fused_norm[0]).squeeze()
                    text_visual_embeddings[i, subj_end:pred_end] = self.lan_img_avgpool(object_visual_feat[i,0] * spo_fused_norm[1]).squeeze() # spo_fused[1]
                    text_visual_embeddings[i, pred_end:obj_end] = self.lan_img_avgpool(object_visual_feat[i,0] * spo_fused_norm[2]).squeeze() # spo_fused[1]
        ...
        vl_embeddings[grid_pos == object_end] = self.end_embedding(_zero_id) # '[END]'
        ...
        if self.enhanced_img_feature and object_visual_feat is not None and spo_len is not None:
            return embeddings, mask, grid_pos < text_end, (grid_pos >= text_end) & (grid_pos < object_end), spo_fused_masks
        else:
            return embeddings, mask, grid_pos < text_end, (grid_pos >= text_end) & (grid_pos < object_end), None

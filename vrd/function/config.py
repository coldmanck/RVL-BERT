...
_C.DATASET.ANSWER_VOCAB_SIZE = 71 # or 71 if '__background__' added
...
_C.DATASET.IMG_FLIPPED = False

_C.DATASET.ALL_PROPOSALS_TEST = False
_C.DATASET.MAX_NB_OF_OBJ = 15

# ------------------------------------------------------------------------------------- #
# Common dataset options for VRD
# ------------------------------------------------------------------------------------- #

_C.TRAIN_VAL_PATH = 'data/vrd/train.json' # 'data/vrd/detections_train_val_with_split.json'
_C.TEST_PATH = 'data/vrd/test.json' # 'data/vrd/detections_test_with_split.json'
_C.TRAIN_VAL_DATAPATH = 'data/vrd/annotations_train.json' # annotations_train_new.json
# _C.VAL_DATAPATH = 'data/vrd/annotations_val_new.json'
_C.TEST_DATAPATH  = 'data/vrd/annotations_test.json' # 'data/vrd/new_annotations_train_val.json'

_C.IMAGEPATH = 'data/vrd/images'
_C.MAX_PHRASE_LEN = 2 # no use for now
_C.PREDICATE_CATEGORIES = ['__background__', 'on', 'wear', 'has', 'next to', 'sleep next to', 'sit next to', 'stand next to', 'park next', 'walk next to', 'above', 'behind', 'stand behind', 'sit behind', 'park behind', 'in the front of', 'under', 'stand under', 'sit under', 'near', 'walk to', 'walk', 'walk past', 'in', 'below', 'beside', 'walk beside', 'over', 'hold', 'by', 'beneath', 'with', 'on the top of', 'on the left of', 'on the right of', 'sit on', 'ride', 'carry', 'look', 'stand on', 'use', 'at', 'attach to', 'cover', 'touch', 'watch', 'against', 'inside', 'adjacent to', 'across', 'contain', 'drive', 'drive on', 'taller than', 'eat', 'park on', 'lying on', 'pull', 'talk', 'lean on', 'fly', 'face', 'play with', 'sleep on', 'outside of', 'rest on', 'follow', 'hit', 'feed', 'kick', 'skate on'] # added '__background__'
_C.OBJECT_CATEGORIES = ['person', 'sky', 'building', 'truck', 'bus', 'table', 'shirt', 'chair', 'car', 'train', 'glasses', 'tree', 'boat', 'hat', 'trees', 'grass', 'pants', 'road', 'motorcycle', 'jacket', 'monitor', 'wheel', 'umbrella', 'plate', 'bike', 'clock', 'bag', 'shoe', 'laptop', 'desk', 'cabinet', 'counter', 'bench', 'shoes', 'tower', 'bottle', 'helmet', 'stove', 'lamp', 'coat', 'bed', 'dog', 'mountain', 'horse', 'plane', 'roof', 'skateboard', 'traffic light', 'bush', 'phone', 'airplane', 'sofa', 'cup', 'sink', 'shelf', 'box', 'van', 'hand', 'shorts', 'post', 'jeans', 'cat', 'sunglasses', 'bowl', 'computer', 'pillow', 'pizza', 'basket', 'elephant', 'kite', 'sand', 'keyboard', 'plant', 'can', 'vase', 'refrigerator', 'cart', 'skis', 'pot', 'surfboard', 'paper', 'mouse', 'trash can', 'cone', 'camera', 'ball', 'bear', 'giraffe', 'tie', 'luggage', 'faucet', 'hydrant', 'snowboard', 'oven', 'engine', 'watch', 'face', 'street', 'ramp', 'suitcase'] # haven't added '__background__'
...
# Spatial Model
_C.NETWORK.USE_SPATIAL_MODEL = False
_C.NETWORK.USE_COORD_VECTOR = False
# _C.NETWORK.SPA_HIDDEN_DIM = 768
_C.NETWORK.SPA_ADD = False
_C.NETWORK.USE_SPA_WEIGHT = False
_C.NETWORK.SPA_FEAT_WEIGHT = 0.7
_C.NETWORK.SPA_CONCAT = False
_C.NETWORK.SPA_ONE_MORE_LAYER = False
# use predicate classification
_C.NETWORK.USE_PREDICATE_CLASSIFICATION = False
# use UVTransE
_C.NETWORK.USE_UVTRANSE = False
_C.NETWORK.UVT_ADD = True
_C.NETWORK.UVT_FEAT_WEIGHT = 0.5
_C.NETWORK.UVT_CONCAT = False
...
_C.NETWORK.VLBERT.use_img_region_order = False # default to False
...
_C.NETWORK.VLBERT.predict_on_cls = False
_C.NETWORK.VLBERT.NO_PREDICATE = False

_C.NETWORK.VLBERT.TRAIN_FROM_SCRATCH = False
_C.NETWORK.VLBERT.vlbert_frozen = False
_C.NETWORK.VLBERT.vlbert_frozen_embedding_LayerNorm = False
_C.NETWORK.VLBERT.vlbert_frozen_encoder = False
_C.NETWORK.VLBERT.vlbert_unfrozen_layers = 0
_C.NETWORK.VLBERT.ENHANCED_IMG_FEATURE = False
_C.NETWORK.VLBERT.mask_weight = 0.5
_C.NETWORK.VLBERT.mask_loss_sum = False
_C.NETWORK.VLBERT.mask_loss_mse = False
...
_C.TRAIN.SAMPLE_RELS = -1 # -1 means no sample performed

...
from common.metrics import vrd_metrics
...
from vrd.function.val import do_validation
...
def train_net(args, config):
    ...
    if args.dist:
        ...
        if config.NETWORK.VLBERT.vlbert_frozen:
            ...
            if config.NETWORK.VLBERT.vlbert_unfrozen_layers != 0:
                for p in model.vlbert.encoder.layer[-config.NETWORK.VLBERT.vlbert_unfrozen_layers:].parameters():
            ...
            if config.NETWORK.USE_SPATIAL_MODEL:
                if not config.NETWORK.USE_COORD_VECTOR:
            ...
            if config.NETWORK.USE_UVTRANSE:
                for p in model.union_vec_fc.parameters():
                    p.requires_grad = True
                for p in model.uvt_fusion_linear.parameters():
                    p.requires_grad = True
                for p in model.uvt_linear.parameters():
                    p.requires_grad = True
        ...
        if config.NETWORK.VLBERT.vlbert_frozen:
            ...
            if config.NETWORK.USE_SPATIAL_MODEL:
                if config.NETWORK.USE_COORD_VECTOR:
                    for p in model.loc_fcs.parameters():
                        p.requires_grad = True
                else:
                    for p in model.simple_spatial_model.parameters():
                        p.requires_grad = True
        ...
        # loader
        train_loader = make_dataloader(config, mode=config.DATASET.TRAIN_IMAGE_SET, distributed=False)
        val_loader = make_dataloader(config, mode=config.DATASET.VAL_IMAGE_SET, distributed=False)
        test_loader = make_dataloader(config, mode=config.DATASET.TEST_IMAGE_SET, distributed=False)
        train_sampler = None
    ...
    # metrics
    train_metrics_list = [vrd_metrics.Accuracy(allreduce=args.dist,
                                                   num_replicas=world_size if args.dist else 1)]
    val_metrics_list = [vrd_metrics.Recall50(allreduce=args.dist,
                                                 num_replicas=world_size if args.dist else 1,
                                                 remove_bg=config.TRAIN.SAMPLE_RELS != -1)]
    for output_name, display_name in config.TRAIN.LOSS_LOGGERS:
        train_metrics_list.append(
            vrd_metrics.LossLogger(output_name, display_name=display_name, allreduce=args.dist,
                                   num_replicas=world_size if args.dist else 1))
    ...
    if config.VAL.NO_VAL:
        validation_monitor = None
    else:
        validation_monitor = ValidationMonitor(do_validation, val_loader, val_metrics,
                                            host_metric_name='Recall50',
                                            label_index_in_batch=config.DATASET.LABEL_INDEX_IN_BATCH)
    if config.TEST.NO_TEST:
        testing_monitor = None
    else:
        testing_monitor = ValidationMonitor(do_validation, test_loader, val_metrics,
                                            host_metric_name='Recall50',
                                            label_index_in_batch=config.DATASET.LABEL_INDEX_IN_BATCH,
                                            do_test=True)
    ...
    train(model, optimizer, lr_scheduler, train_loader, train_sampler, train_metrics,
        ...
        gradient_accumulate_steps=config.TRAIN.GRAD_ACCUMULATE_STEPS, testing_monitor=testing_monitor, debug=config.TRAIN.DEBUG)

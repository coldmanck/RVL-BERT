...
from common.metrics import spasen_metrics
...
from spasen.function.val import do_validation
...
def train_net(args, config):
    ...
    # metrics
    train_metrics_list = [spasen_metrics.Accuracy(allreduce=args.dist,
                                                   num_replicas=world_size if args.dist else 1)]
    val_metrics_list = [spasen_metrics.Accuracy(allreduce=args.dist,
                                                 num_replicas=world_size if args.dist else 1)]
    for output_name, display_name in config.TRAIN.LOSS_LOGGERS:
        train_metrics_list.append(
            spasen_metrics.LossLogger(output_name, display_name=display_name, allreduce=args.dist,
                                   num_replicas=world_size if args.dist else 1))
    ...
    train(model, optimizer, lr_scheduler, train_loader, train_sampler, train_metrics,
        ...
        gradient_accumulate_steps=config.TRAIN.GRAD_ACCUMULATE_STEPS, testing_monitor=testing_monitor)
